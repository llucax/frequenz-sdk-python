...
class GraphGenerator:
    ...
    def component(
        ...
        comp_type: InverterType | None = None,
    ) -> Component:
...
def test_graph_generator_simple() -> None:
    ...
    meters = list(graph.components(filter_by_types={Meter}))
    ...
    inverters: list[Inverter] = cast(
        list[Inverter],
        list(graph.components(filter_by_types={Inverter})),
    )
    ...
    assert len(graph.components(filter_by_types={Battery})) == 2
    assert len(graph.components(filter_by_types={EvCharger})) == 1
...
def test_graph_generator_no_grid_meter() -> None:
    ...
    meters: list[Meter] = cast(
        list[Meter],
        list(graph.components(filter_by_types={Meter})),
    )
    assert len(meters) == 1
